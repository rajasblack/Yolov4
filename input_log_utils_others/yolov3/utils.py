...
from input_log_utils_others.yolov3.yolov4 import *
...
def Load_Yolo_model(model_path=""):
    ...
    if YOLO_FRAMEWORK == "tf": # TensorFlow detection
        ...
        if model_path:
            ...
            yolo.load_weights(model_path.strip(".index")) # use custom weights
        else:
            if YOLO_CUSTOM_WEIGHTS == False:
                ...
                yolo = Create_Yolo(input_size=YOLO_INPUT_SIZE, CLASSES=TRAIN_CLASSES)
                yolo.load_weights(f"./input_log_utils_others/checkpoints/{TRAIN_MODEL_NAME}") # use custom weights
...
def draw_bbox(filename, output_result, image, predict_path,bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    ...
    for i, bbox in enumerate(bboxes):
        file1 = open(predict_path + output_result,"a") 
        ...
        if show_label:
            ...
            bbox = "{},{},{},{},{},{}".format(x1,y1,x2,y2,class_ind,score)
            string = filename + " "+bbox+"\n"
...
def detect_image(filename, output_result, Yolo, predict_path,image_path, output_path,input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.5, iou_threshold=0.45, rectangle_colors=''):
    ...
    image = draw_bbox(filename, output_result, original_image, predict_path,bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors)
    # CreateXMLfile("XML_Detections", str(int(time.time())), original_image, bboxes, read_class_names(CLASSES))
#     print("{},{},{},{},{}".format(x1,y1,x2,y2,0))
